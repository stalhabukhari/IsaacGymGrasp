...
from isaacgymgrasp.grasp_sim.objects.object import SimpleObject
from isaacgymgrasp.grasp_sim.robots.gripper_only import GripperOnly
from isaacgymgrasp.utils.geometry_utils import Transform_2_H
...
class GraspingGymEnv:
    ...
    def __init__(self, gym, sim, env, isaac_base, curr_env_number, args=None):
        ...
        self.data_dir = args["obj_args"]["data_dir"]
        self.n_dofs = 16
    ...
    def _set_args(self, args):
        ...
        if "obj_or" not in args["obj_args"]:
            args["obj_args"]["obj_or"] = np.array([0.0, 0.0, 0.0, 1.0])
    ...
    def _load_table(self):
        ...
        table_path = osp.join(ENVDATADIR, "table/table.urdf")
        table_asset = self.gym.load_asset(self.sim, "", table_path, asset_options)
        ...
        table_handle = self.gym.create_actor(
            self.env, table_asset, table_pose, "table", self.curr_env_number
        )
        return table_handle
    ...
    def _load_gripper(self, obj_pose):
        ...
        pose.r = gymapi.Quat(grasp_quat[0], grasp_quat[1], grasp_quat[2], grasp_quat[3])

        gripper = GripperOnly(
            self.gym, self.sim, self.env, self.isaac_base, self.curr_env_number, pose
        )
