...
from isaacgymgrasp.utils.geometry_utils import H_2_Transform
from isaacgymgrasp import ENVDATADIR
...
class GripperOnly:
    def __init__(
        ...
        self.handle = self.gym.create_actor(
            ...
            "franka",
            group=env_number,
            filter=collision_group,
            segmentationId=segmentationId,
        )
        self.hand_handle = self.gym.find_actor_rigid_body_handle(
            self.env, self.handle, "panda_hand"
        )
        self.base_handle = self.gym.find_actor_rigid_body_handle(
            self.env, self.handle, "world"
        )

        ## Set initial joint configuration ##
        curr_joint_positions = self.gym.get_actor_dof_states(
            self.env, self.handle, gymapi.STATE_ALL
        )
        curr_joint_positions["pos"][-1] = 0.04
        curr_joint_positions["pos"][-2] = 0.04

        self.gym.set_actor_dof_states(
            self.env, self.handle, curr_joint_positions, gymapi.STATE_ALL
        )
        ...
        self.gym.set_actor_dof_position_targets(
            self.env, self.handle, curr_joint_positions["pos"]
        )
    ...
    def set_assets(self):
        # Load franka asset
        franka_asset_file = osp.join(
            ENVDATADIR, "hand_only/robots/franka_panda_hand.urdf"
        )
        ...
        return self.gym.load_asset(self.sim, "", franka_asset_file, asset_options)
    ...
    def get_state(self, rb_states, dof_pos, dof_vel):
        ...
        self.base_state_dict = {
            "base_pos": base_pos,
            "base_rot": base_rot,
            "base_vel_p": base_vel_p,
            "base_vel_r": base_vel_r,
        }
        ...
        hand_state_dict = {
            "hand_pos": hand_pos,
            "hand_rot": hand_rot,
            "hand_vel_p": hand_vel_p,
            "hand_vel_r": hand_vel_r,
        }

        self.robot_state = [dof_pos, dof_vel]
        ## Fingers state
        finger_state_dict = {
            "r_finger_pos": dof_pos[-1],
            "l_finger_pos": dof_pos[-2],
            "r_finger_vel": dof_vel[-1],
            "l_finger_vel": dof_vel[-2],
    ...
    def _init_cntrl(self):
        ...
        hand_pose = self.gym.get_actor_rigid_body_states(
            self.env, self.handle, gymapi.STATE_POS
        )["pose"][:][-4]
