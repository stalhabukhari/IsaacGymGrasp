...
# from lib_nn.torch_utils import to_numpy, to_torch
...
class EvaluatePointConditionedGeneratedGrasps:
    ...
    def pointcloud_conditioning(self):
        ...
        P = mesh.sample(1000)
        P = torch.from_numpy(P).float().to(self.generator.device)
        rot = (
            torch.from_numpy(R.from_quat(self.q).as_matrix())
            .float()
    ...
    def measure_empirircal_dist_distance(self, H_sample=None):
        ...
        for k in range(self.args["empirical_dist_episodes"]):
            ...
            xyz_dist = (
                (p_eval[None, ...] - p_sample[:, None, ...]).pow(2).sum(-1).pow(0.5)
            )
