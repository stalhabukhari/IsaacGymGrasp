...
class GraspSuccessEvaluator:
    ...
    def _get_args(self, obj_ids, obj_types="Mug", scales=1.0, rotations=None):
        args = []
        for obj_id, obj_type, scale, rotation in zip(
            obj_ids, obj_types, scales, rotations
        ):
            obj_args = {
    ...
    def _compute_success(self, s):
        for si in s:
            ...
            distance = (hand_pos - obj_pos).pow(2).sum(-1).pow(0.5)
...
class GraspController:
    ...
    def __init__(
        self, env, hand_cntrl_type="position", finger_cntrl_type="torque", n_envs=0
    ):
        self.env = env
        ...
        self.squeeze_force = 0.6
        self.hold_force = [self.squeeze_force] * n_envs

        self.r_finger_target = [0.0] * n_envs
        self.l_finger_target = [0.0] * n_envs
    ...
    def _approach(self, state, idx=0):
        ...
        r_error = state[0]["r_finger_pos"] - 0.04
        l_error = state[0]["l_finger_pos"] - 0.04
        ...
        error = pose_error.pow(2).sum(-1).pow(0.5)
        if error < 0.005:
            print("start grasp")
    ...
    def _squeeze(self, state, idx=0):
        ...
        action = {"grip_control_type": "torque", "des_grip_torque": des_finger_torque}
